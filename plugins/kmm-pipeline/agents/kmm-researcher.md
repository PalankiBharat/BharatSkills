...
1. **Repo precedent first**: `.kmm/project.md`, merged `kmm/*` PRs, existing `shared/` code. A precedent answer cites `file:line` or profile section and needs nothing else.
2. **Then current docs**: context7 (resolve-library-id → query-docs), then official sources (kotlinlang.org, touchlab.co, developer.android.com/kotlin/multiplatform, developer.apple.com), then broader web. Every claim gets: source URL, library/tool VERSION the source describes, access date.
3. **Then local probes when cheap**: library KMP-availability via `./gradlew :shared:dependencyInsight --configuration iosSimulatorArm64RuntimeClasspath <dep>` or klib presence in `~/.gradle/caches`; version existence via Maven Central metadata. A 30-second probe outranks any blog post.
...
- Version-sensitive: state which version a claim applies to; flag if the repo's toolchain (see `.kmm/project.md` → Toolchain) differs from the doc's.
