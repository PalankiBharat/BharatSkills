...
Binding rules for every agent that touches code during a KMM migration. The orchestrator includes this file's path in every dispatch; read it fully before your first edit. Violating the letter of a rule is violating the rule — there is no "spirit" escape hatch. Rules marked HOOK are also machine-enforced; a hook denial is a Law violation already in progress, not an obstacle to work around.

## 1. Moves are `git mv` — HOOK

Relocating a file from `:app` (or any module) into `:shared` is always `git mv`, never copy + delete, never `Write` of a new file with old content. After the move, `git diff -M90% --name-status` must show `R` (rename), not `D`+`A`. Test files promote the same way (`app/src/test` → `shared/src/commonTest`).
...
Every step ends with: `:app` assembles and the step's affected unit tests pass; if `:shared` was touched, `:shared:compileKotlinIosArm64` passes (the repo's iOS gate — `linkPod*` standalone and `--rerun-tasks` are known-broken here; see `.kmm/project.md` → Gradle gotchas). Verify tests actually ran via JUnit XML, not task output. Red gate → fix or STOP; never proceed on red, never weaken an assertion to go green.
...
- **Quarantine, don't fix**: a pre-existing unrelated broken/flaky test gets `@Ignore` + one-line reason + an entry in the report's out-of-scope list. Fixing it inline is a Rule 3 violation.
- **Frozen once green**: after G2, baseline edits require a written exception in `blockers/` approved at a gate — append-only, with reason and sign-off.
