...
All pipeline state is plain files in the MAIN repo at `.kmm/migrations/` (excluded from git via `.git/info/exclude`; the exclude file lives in the common git dir, so it covers worktrees too). The committed `.kmm/project.md` is the repo's durable knowledge profile — read it at start, append learnings at close, never duplicate it into state.
...
Step status: `pending | in-progress | done | blocked`. Phases: 0 preflight · 1 scope · 2 research · 3 plan · 4 execute · 5 ios · 6 verify · 7 ship.

## journal.ndjson

One JSON object per line: `{"ts": "<UTC ISO>", "phase": 4, "step": "S2", "type": "dispatch|commit|gate|blocker|phase-done|note", "sha": "...", "note": "..."}`. Workers append their own `commit` events; the orchestrator verifies on receipt and appends `phase-done`. Timestamps via `date -u +%Y-%m-%dT%H:%M:%SZ`. A step without journal entries did not happen (Law Rule 10).

## Resume algorithm

On any invocation with `ACTIVE` present (or `resume` argument):

1. Read `ACTIVE` → state dir → `state.json` + last 30 journal lines.
...
- Single writer per file: orchestrator owns `state.json`/`plan.md`/`contract.md`; a worker appends to `journal.ndjson` and writes its own `blockers/<n>.md`, nothing else. State writes are atomic (write temp, `mv` over).
