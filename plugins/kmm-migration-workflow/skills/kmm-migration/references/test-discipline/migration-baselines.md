...
2. **Every server-decoded DTO field is nullable or defaulted.** Gson sets an absent field to null/default; kotlinx throws `MissingFieldException` unless the field has `?` or `= default`. **`isLenient`/`coerceInputValues` do NOT help here — only `?` / `= default` does.** Tripwire: a `decode("{}")` test per response DTO.
   - **Unknown *enum value* is a distinct trap from a missing field.** When the BE sends an enum string not in the Kotlin enum, **Gson puts `null` into the (often non-null) field and does not throw**; kotlinx throws `SerializationException` *unless* `coerceInputValues = true` **and** the field is nullable-or-defaulted (then it coerces the unknown value to the default/null). So an unknown-enum field needs **both** the lenient `Json` *and* `?`/`= default` — and you must decide the master-equivalent target explicitly (unknown → `null`, or → a sentinel). Reproducing Gson's silent unknown-enum→null is equivalence-preserving; surfacing it as a throw is a behavior change. **Don't infer this from source-reading** — characterize the OLD Gson behavior empirically against a real payload carrying the unknown value (the snapshot/golden in §"Snapshot / golden files"), because Gson's null-on-unknown is invisible in the type signature.
3. **Keep master's exact wire type — no opportunistic `String`→`Double`/`Long` "upgrades."** A migration is behavior-preserving; a type refactor is a separate PR with its own BE verification (a type change can break an edge/stringly payload and force `.toString()` round-trips on consumers). Reinforces Phase D's verbatim-old-behavior pre-flight.
...
5. **Never swallow a decode/parse failure.** An empty `onFailure {}` / empty `catch` turns a strict-decode error into a silent failure (e.g. an infinite status-poll that hides for a full timeout). Log/surface it — a strict-decode bug must be loud, not a silent loop.
