...
- **Storage ownership** — does each platform own its native DB (native-per-platform: e.g., Android keeps ObjectBox/Room-Android, iOS gets its own, commonMain holds plain data classes + interfaces), or is storage shared (e.g., Room KMP / SQLDelight everywhere)? This gates the entire entity/DAO plan. **Surface the interface-vs-impl placement option early:** once a capability is `interface (commonMain) + impl`, the impl can live in *any* downstream module that sees the interface — so a future Room/iOS impl can land in `:shared`/`iosMain` without moving the interface or its module. Raising this up front short-circuits the Room-everywhere-vs-platform-owned and module-move debates (the user reached it first in a prior session, after several round-trips).
