...
- **Re-validate proportional to the fix** — reuse Phase F.6's mechanical re-validation scope: a surgical fix (≤5 LOC, single file, no new identifiers) re-runs F.3 (build + tests) only; a non-surgical fix returns to F.1 and re-validates fully. Announce the scope chosen with a one-line justification.
