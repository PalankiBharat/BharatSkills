...
3. **Verify-before-offering (NON-NEGOTIABLE).** Before any option reaches the user, the orchestrator **self-verifies every subagent-reported API/library/module fact the options rest on** — read the actual signature from source, confirm the module boundary (a cross-module `internal` is useless to a `commonTest` in another module), confirm the library version exists and fits the repo's Kotlin version. An option built on an unverified claim is not presented. **This extends to subagent *semantic* verdicts, not just API/version facts:** never carry a single subagent's verdict on exception/`toString()`/error-message semantics, or two subagents' *conflicting* facts (e.g. divergent per-service timeouts), straight into the plan or a triage — reconcile against the **live/old code path** yourself first (`error(Throwable(s))` vs `error(s)` surface different strings; one builder's 15s ≠ another's dead 30s). A contradiction is never recorded; it's resolved. *Why: across two sessions, users wasted whole decision rounds choosing options that turned out impossible — a Hilt approach that failed at compile on an untested KSP edge case, and the N1 fix that needed three rounds because each option's infeasibility surfaced only after a subagent tried it; a contradictory message-semantics verdict in Phase H had to be settled by reading master directly. The cost of a quick self-verify (read one file, check one boundary) is tiny next to a wasted round-trip.*
...
4. **Verify the cross-phase ledgers exist before continuing.** `coverage.md` and `phase-d-followups.md` are relied on by every later phase but may never have been created (a prior in-flight Phase B resumed with both missing). If absent for an in-flight session, create them from `scope.md` + `plan.md` (coverage rows from the manifest + per-file Phase D plan; followups from the plan's deferred-baseline list) before picking up work — don't proceed on a missing ledger.
5. Pick up at the next pending task identified by the report.
