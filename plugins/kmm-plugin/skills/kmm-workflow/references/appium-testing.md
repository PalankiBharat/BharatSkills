...
For Compose Multiplatform on iOS, `Modifier.testTag("name")` does NOT automatically map to accessibility identifiers. You must add `Modifier.semantics { testTagsAsResourceId = true }` on Android AND explicitly set `.accessibilityIdentifier("name")` on iOS CMP views. Verify selector availability via hierarchy dump before writing flows. Android can reliably use `id` selectors from `testTag` or resource IDs without this fallback chain.
