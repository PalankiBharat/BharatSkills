...
| `NSData.dataWithContentsOfFile()` / `NSData.create(contentsOfFile:)` | Neither form resolves in K/N — ObjC factory methods are unreliably exported | `NSFileManager.defaultManager.contentsAtPath()` or okio `FileSystem.SYSTEM.read()` | If okio is already a dependency (e.g., via DataStore), prefer okio for ALL file operations in iosMain — avoids K/N Foundation interop entirely. |
| `NSURL.lastPathComponent` | May not resolve in K/N even with correct imports | `path.substringAfterLast('/')` | Pure Kotlin string ops are always reliable in K/N. |
| `NSFileManager.attributesOfItemAtPath()[NSFileSize]` | Returns `Any?`, requires safe conversion to avoid type casting | `okio.FileSystem.SYSTEM.metadata(path.toPath()).size` | okio metadata returns typed `Long?` directly. |
| `CIFilter.filterWithName()` | Unresolved in K/N — ObjC factory method exposure is unreliable | Stub impl in iosMain; Swift host app provides real rendering via Koin | Applies to ALL CoreImage/CoreGraphics rendering: QR codes, image processing, bitmap manipulation. K/N interop for these frameworks wastes compilation cycles. |
| `CGBitmapContextCreate` / `CGBitmapContextRelease` | Release doesn't exist in K/N; Create has UInt parameter ambiguity | Stub impl; use Swift for bitmap rendering | `CGBitmapContextGetData()` returns `Any` not `CGImageRef?`. |
| `kCGImageAlphaPremultipliedLast` | Not exported to K/N | Use raw value `1u` if absolutely needed, but prefer stub impl | CoreGraphics constants are inconsistently exported. |
| `CIContext.createCGImage()` | Returns `Any` not `CGImageRef?` — type mismatch | Stub impl | Combined with other CG/CI issues, full rendering in K/N is impractical. |
| `NavHost` / `rememberNavController` | AndroidX Navigation Compose only — not available in CMP | Use state-based navigation: sealed class/`when` mapping state → composable | CMP navigation must be implemented via state machines, not AndroidX NavHost. |
| `ModalBottomSheet` `skipPartiallyExpanded` | Android Compose only parameter — not available in CMP | Remove the parameter; CMP `ModalBottomSheet` supports `onDismissRequest` + `sheetState` only | CMP modal sheets have a reduced parameter set vs AndroidX. |
| `PlatformTextStyle(includeFontPadding = false)` | Android Compose only — not available in CMP | Remove from `TextStyle`; CMP handles font padding differently | No CMP equivalent — just omit. |
| `Icons.Filled.Help` | Not available in CMP material-icons-extended | Use `Icons.Filled.Info` or a custom vector asset | Not all Material Icons are available in CMP. Check availability before using. |
| `R.drawable.*` / `R.string.*` | Android-only resource system — unavailable in commonMain/CMP | Two strategies: (1) Pass `Painter`/`ImageVector`/`String` as composable parameters (preferred for host-app-supplied assets). (2) Use CMP `compose.components.resources` (`Res.font.*`, `Res.drawable.*`) for bundled assets. | Create a dedicated assets data class (e.g., `ScreenAssets(logo: Painter, title: String)`) for host-app-supplied resources. Never reference `R.*` in shared composables. |
