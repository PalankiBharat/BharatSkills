...
- For each level (leaves first):
  ...
  - **Original deletion:** After all agents at a level complete, the ORCHESTRATOR deletes originals — agents cannot reliably delete files.
  - **Pre-delete originals:** Before dispatching agents for a level, delete ALL `src/main/java` originals for files being migrated. Originals are committed in the previous checkpoint — safe to remove. Prevents duplicate class errors when agents write to commonMain.
