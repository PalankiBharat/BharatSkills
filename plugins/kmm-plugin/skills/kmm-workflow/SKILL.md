...
- **Improve** → read open GitHub issues with `skill:kmm-workflow` label, classify learnings, create a branch on the skill source repo (e.g., `~/dev/claude-code-skills`), consolidate into existing skill files (NEVER append — rewrite to absorb), measure file growth, bump patch version in `plugin.json`, raise PR. All edits happen on the skill source repo — NEVER edit the local plugin installation under `~/.claude/plugins/`. Do NOT update the `description` field in `plugin.json` or SKILL.md frontmatter unless the user explicitly asks. See `references/self-improvement.md`.
