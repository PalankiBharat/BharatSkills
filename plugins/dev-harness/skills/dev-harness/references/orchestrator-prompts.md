...
`harness-init.sh` opens a tmux window `harness-<slug>-<date>` **in your current session**; each pane runs `agent-pane.sh <role>` → a **persistent interactive `claude --agent <persona>`** (bypass perms). The pane's model is **pinned explicitly** by `role_model` (lib.sh), never left to the user's default: deep panes (orchestrator/tech-lead/architect) launch `--model opus`; the build lanes (dev/qa) default to `opusplan` (a `/model`-only mode, so harness-init switches them post-boot via send-keys). Override per role with `HARNESS_MODEL_<ROLE>` (e.g. `HARNESS_MODEL_DEV=sonnet`) or globally with `HARNESS_MODEL`. `send.sh` writes the full instruction to `.harness/<role>/inbox.md`, sets `status=working`, and **nudges the pane** (`send-keys` the trigger, pause, then a **separate `Enter`** — Claude's TUI won't submit a combined "text Enter"). The agent reads its inbox, works **visibly**, runs long commands via `bash .harness/run <role> -- <cmd>`, then signals `bash .harness/done <role>` as its last action. The Orchestrator polls with `poll.sh --settle`; a file-based watchdog covers lost nudges / stalls. Leads dispatch their **sonnet workers** via the Agent tool (`subagent_type: dev-harness:bharat-dev` / `dev-harness:bharat-qa`) — the juniors' `model: sonnet` is honoured, so the typing is cheap. Persona→role: manish=tech-lead · mohit-dev=dev · rohit=qa · mohit-arch=architect.
