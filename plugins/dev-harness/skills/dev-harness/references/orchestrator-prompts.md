...
| `--auto` | skip a review gate only when its `questions.json` has clarifications alone (recommended defaults apply); a **blocker always gates to the human**. Never the security rails; small stories only. |
