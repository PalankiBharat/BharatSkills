...
**Prove parity, don't claim it** (full workflow: `references/figma-parity.md` in the dev-harness skill). When the phase carries a Figma link: **(1)** detect the project's **existing** headless screenshot tool (Google CPST / Roborazzi / Paparazzi — detection table + per-tool tasks in `figma-parity.md`) and use it **in its own idiom**; never install a second screenshot stack beside one that exists. Only a project with **no** headless tool gets Google's CPST added (one-time, test-only, zero APK impact; note the build change in the handoff; KMP module → it attaches to the Android target and previews can call `commonMain` composables). **(2)** Per Figma frame, a frame-sized preview/test with fixed fake data, written in the chosen tool's idiom. **(3)** Render headless (the tool's **record task** — seconds, no emulator), export the frame (`bash .harness/figma-parity export …`; no FIGMA_TOKEN → ask), diff (`bash .harness/figma-parity diff … .harness/artifacts/parity/<screen>/`). **(4)** Iterate until the heatmap is quiet, then list each sheet + DIFF_PCT in `dev-handoff.md` — a Figma phase without parity sheets is not done. **(5)** The human reviews your sheets on the parity page; their `PARITY REVIEW` block comes back verbatim — fix every `needs-changes` screen exactly as commented and regenerate only those sheets. After approval the renders are goldens: keep the tool's **verify task** (e.g. `validateDebugScreenshotTest`, `verifyRoborazziDebug`, `verifyPaparazziDebug`) green in every later chunk.

**Parity HARD RULES — the Orchestrator file-checks all of this, so skipping cannot pass:** every sheet and `diff-pct.txt` must be the output of `bash .harness/figma-parity` — never hand-write `diff-pct.txt`, never assemble a sheet by hand, never report a DIFF_PCT the script didn't print. No `FIGMA_TOKEN` → `bash .harness/done dev blocked` and ask; a guessed token, a skipped export, or "parity looks fine to me" without the script's output is a defect, not a shortcut. These steps are not optional even when the UI "obviously matches".
