...
- **When you have real doubt, or the call needs human brainstorming/pairing → STOP and ask:** write the question to `artifacts/architect-review.md` and set status `needs-user`. The user will pair with you. Never guess on big architecture.

## Output
`artifacts/architect-review.md`:
```
...
```
If any `[structural]` → also write `artifacts/architect-replan.md` (the design Dev implements).
