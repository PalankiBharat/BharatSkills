...
5. **Build per phase — separate lanes.** Dev implements the approved `design.md` test-first (`EXPECT`: code + `dev-handoff.md`); **code/unit tests are Dev's own — never ask Dev to do device/QA work, and never put render/ST/PASS-FAIL/screenshots in a Dev dispatch.** If the story/spec has a **Figma link or UI**, the dispatch states **`figma-to-compose` is REQUIRED** (Dev may not skip it). **A Dev `blocked` that carries a question/doubt is a user gate, not a crash** — do NOT answer it yourself; surface the exact question to the user (needs-user), null `in_flight`, pause, and resume via `.harness/answer dev` once they reply. Then `send qa "VERIFY phase N — real-user + edge-case journeys; fill .harness/artifacts/qa-cases.md (every applicable category) and run the Maestro flows on the locked emulator. EXPECT: .harness/artifacts/qa-report.md"` (QA is manual/UI only; it never touches app code). A phase completes only when `dev-handoff.md`, a covered `qa-cases.md`, **and** a QA-PASS `qa-report.md` all exist — a report that tested only the happy path is not a pass; bounce it back. QA FAIL → re-dispatch Dev with the report (fix budget 7/phase).
...
7. **Final QA regression (feature).** Before Done, one whole-feature pass: `send qa "FINAL regression — every qa-cases.md category across the WHOLE feature, end-to-end as a real user. EXPECT: .harness/artifacts/qa-report.md (final)"`. Must be green over the full checklist, not just the last phase. (Small change / bug fix: the step-6 final QA covers this — skip.)
