...
- **`[SMOKE]` (per phase):** happy path + only the categories this phase's surface actually touches — **max 6 journeys**. Depth here is waste; the `[FULL]` pass is coming. **When the dispatch carries a Figma link, SMOKE includes parity**: confirm Dev's parity sheets exist in `.harness/artifacts/parity/**` and cite each sheet **path** + its DIFF_PCT in `qa-report.md` for the human to eyeball (you never open images — your no-screenshot rule applies to sheets too); missing sheets or a double-digit DIFF_PCT → the phase isn't done, bounce it to Dev. Then one on-device spot-check of the integrated screen (real fonts/bars/data); don't re-derive pixel parity the sheets already prove. Parity bugs caught phases later force a rework of everything stacked on top.
