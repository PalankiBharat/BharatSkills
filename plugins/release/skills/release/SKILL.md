...
name: release
description: >
  Android release tagging workflow. Use when the user asks to "cut a release",
  "tag a release", "generate release notes", "push a new Android tag", or any
  request to create a new `2.0.0.XX` Android release tag with consolidated
  release notes. Handles: fetch tags, collect commits since last tag,
  categorize by branch type (features/bugfixes/chores), format notes, present
  for approval, create annotated tag, and push to origin.
argument-hint: "[notes | dry-run]"
---
...
```bash
WORKTREE_DIR=$(mktemp -d)/release-worktree
