...
| `kmm-migration-workflow` | 1.7.0 | Android-to-KMM migration orchestrator with behavioral-equivalence safety — discovery, diagnostic, baseline tests, freeze, `git mv` + surgical edits, validation, PR. Live API knowledge via web + Context7, project conventions in `.kmm/project.md`, diff-confirm scoped to project.md, commit autopilot, per-phase reflective retro. **Phase A carries an iOS Swift interop lens per file** — search-cited, must-vs-want rubric enforced, no SKIE patterns baked in |
...
| `kmm-qa-autopilot` | 0.1.0 | Parity QA for Android-to-KMM migration PRs — proves a migration is behavior-preserving by running latest `master` and the PR head head-to-head. From a PR link: builds two ProductionDebug APKs (same package), boots + locks two visible emulators (master→A, PR→B), one manual prod login each, a no-exclusions heatmap from the master-vs-PR diff, the SAME Maestro flows on both devices, and a view-hierarchy structure + stable-value diff (live prices/charts auto-masked by double-sampling) → per-journey parity verdict. Real-prod-state safety gate holds order/funds/kill-switch flows behind explicit confirmation |
