...
| `dev-harness` | 0.1.1 | `/harness "<story>"` drives a visible 5-pane tmux team (Tech Lead + Senior/Junior Dev + QA Lead/Tester + Review Architect) from a story to a tested, reviewed PR. Persona agents (opus leads dispatch sonnet workers), adaptive flow (feature / small-change / bug-fix), phased delivery, file-mailbox coordination, live streaming panes, two-lane feedback, restart, crash-safe resume, PreToolUse security guard, opt-in OS sandbox (`--sandbox`), and multi-run worktrees (`--worktree`). |
...
| `kmm-migration-workflow` | 1.10.0 | Android-to-KMM migration orchestrator with behavioral-equivalence safety — discovery, diagnostic, baseline tests, freeze, `git mv` + surgical edits, validation, PR, a code-review intake phase (resolve blockers through the workflow), then parity-QA + bug-fixing. Live API knowledge via web + Context7, project conventions in `.kmm/project.md`, diff-confirm scoped to project.md, commit autopilot, **blocking per-phase retro + state-serialization gates**. **Phase A carries an iOS Swift interop lens per file** (search-cited, must-vs-want rubric) and **resolves platform-ownership + verifies SDK iOS-availability from gradle metadata** before Phase D; **iOS klib + cold-KSP compile gates every commonMain touch**. **Concurrency-primitive swaps (e.g. `ConcurrentHashMap`→`Mutex`) are flagged in Phase A and require a multi-caller parity assertion in Phase B**, so a serialized/coalesced behavior change can't be frozen as equivalence |
