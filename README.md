...
| `dev-harness` | 0.1.18 | `/harness "<story>"` drives a visible 6-pane tmux team (a driving Orchestrator pane + Tech Lead Manish + Dev Bharat + QA Rohit + Review Architect Mohit) from a story to a tested, reviewed PR. Per-pane models (opus for orchestrator/tech-lead/architect, opusplan for dev/qa), adaptive flow (feature / small-change / bug-fix), phased delivery, file-mailbox coordination, live streaming panes, two-lane feedback, restart, crash-safe resume, PreToolUse security guard, opt-in OS sandbox (`--sandbox`), and multi-run worktrees (`--worktree`). |
| `figma-to-compose` | 8.0.0 | Generates Jetpack Compose Kotlin code from Figma designs — Compose Multiplatform by default, Android via a flag — with design tokens, drawable assets, and multi-screen batch export. Auto-triggers on any Figma URL in a Kotlin/Android/KMP context; enforces token + composable reuse, render verification, and a review checklist against hallucinated UI |
...
| `kmm-migration-workflow` | 1.14.0 | Android-to-KMM migration orchestrator with strict behavioral-equivalence safety. Phased workflow — discovery, diagnostic, frozen baseline tests, `git mv` + surgical migration, Android+iOS validation, PR, code-review intake, then an in-skill autonomous parity-QA loop (agent-device replay of a runtime golden) that fixes bugs through the workflow until the checklist is green. Live-sourced API knowledge (web + Context7), per-repo conventions in `.kmm/project.md`, diff-confirm gates, commit autopilot, blocking per-phase retro |
...
/plugin install figma-to-compose@bharatskills
```
