...
| `sniper-ops` | 1.2.0 | Ops shortcuts for the sniper-v2-android repo — `share-prod` / `share-staging` (commit + push + release-notes pre-flight then trigger CircleCI build), `install-prod` / `install-staging` (local install to a connected device, then auto-launch the app and tail a crash-only logcat `*:E` scoped to the app's PID in the background), and `heap-dump-pair` (option-driven interview to capture a baseline + t+5min `.hprof` pair so you can diff retained objects in Android Studio Profiler) |
